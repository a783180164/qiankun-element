...
export function shuffle(arr: Array<any>): Array<any> {
  ...
  const rtn = [...arr]; //浅拷贝就足矣，因为只是改变数组元素的下标，与元素内容无关。速度比深拷贝快很多（几倍甚至更多），且数量越多越明显。
 ...
 *
 * */
export function throttle(callback: (...args: any) => void, time: number, immediate: boolean): (this: unknown, ...args: any) => void {
  let previous = 0;
...
export function debounce(callback: (...args: any) => void, time: number, immediate: boolean): (this: unknown, ...args: any) => void {
  let timeout: number | null = null;
  return function(this: unknown, ...args: any){
    const context = this
